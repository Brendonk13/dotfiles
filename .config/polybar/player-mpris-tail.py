...
#            else:
#            try:
#                spotify_running = check_output(['pgrep', '-x', 'spotify']).decode('UTF-8')
#            except:
#                spotify_running = None
#            if spotify_running is None:
                ...
                with open('/home/brendonk/.config/polybar/showSongInfo.txt', 'r') as f: 
                    ...
                    for line in f:
                        if line.startswith("haveClicked"):
                            booll = True
                    if booll:
                        #print a stopped icon if we just clicked ie want to hide shit
                        _printFlush(ICON_INFO)
                    else:
